...
const getData = () => {
    ...
    // Add data to Sorted Set with timestamp as score
    const asynczadd = async (key, score, value) => {
    ...
    (err, data) => {
        ...
        if (data.message == "OK" && data.request_state == "200") {
            ...
            dataArray.map((curr, idx, array) => {
                ...
                // Create datetime string in format "MM/DD/YYYY-HH" for compatibility
                ...
                // Store in new Sorted Set structure: device:{deviceId}:data
                asyncsadd("devices", device).then()
                asynczadd(`device:${device}:data`, timestamp, jsonData).then(() => {
                    // console.log(`[Redis] ZADD device:${device}:data ${timestamp} ${jsonData}`)
                })
            })
        }
